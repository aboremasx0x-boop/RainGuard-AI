...
/* ===============================
   Rain Alert Lifecycle + Live Event Bus Integration
================================ */
...
function getRainAlertStage(city) {
    ...
    if (score >= 70) return { stage: "raining", label: "🔴 Raining | المطر حاضر الآن", color: "#ef4444" };
    if (maxScore >= 70 || flood >= 50 || (cloud >= 80 && humidity >= 70)) return { stage: "high_risk", label: "🟠 High Risk | خطر مرتفع", color: "#f59e0b" };
    if (maxScore >= 50 || flood >= 35 || (cloud >= 65 && humidity >= 60)) return { stage: "developing", label: "🟡 Developing | الحالة تتطور", color: "#facc15" };
    if (maxScore >= 30 || flood >= 25) return { stage: "monitoring", label: "🔵 Monitoring | تحت المتابعة", color: "#38bdf8" };
    return { stage: "cleared", label: "⚪ Cleared | انتهى التنبيه", color: "#94a3b8" };
}
...
function saveRainAlertLifecycle(results) {
    ...
    localStorage.setItem(RAIN_ALERT_LIFECYCLE_KEY, JSON.stringify(activeAlerts));
...
function loadRainAlertLifecycle() {
    try {
        const data = JSON.parse(localStorage.getItem(RAIN_ALERT_LIFECYCLE_KEY) || "[]");
...
function renderAlertTimeline(alertItem) {
    return `<div class="alert-timeline">${getAlertTimelineSteps(alertItem).map(step => `
        <div class="alert-timeline-step ${step.active ? "active" : ""}">
            <span>${step.active ? "✓" : "○"}</span>${step.label}
        </div>`).join("")}</div>`;
}
...
function renderRainAlertLifecycle(alerts = loadRainAlertLifecycle()) {
    ...
    if (!alerts.length) {
        box.innerHTML = `<div style="color:#94a3b8;line-height:1.8;">لا توجد تنبيهات مطر نشطة حالياً.</div>`;
        return;
    }
    box.innerHTML = alerts.map(alertItem => `
        <div class="lifecycle-alert-card" style="border:1px solid ${alertItem.color || "#38bdf8"};">
            <strong style="color:${alertItem.color || "#38bdf8"};">${alertItem.name || "--"}</strong><br>
            ...
            <button type="button" onclick="showRainAlertReason('${String(alertItem.name || "").replace(/'/g, "\\'")}')">سبب التنبيه</button><br>
            <small>آخر تحديث: ${alertItem.updatedAt ? new Date(alertItem.updatedAt).toLocaleTimeString("ar-SA") : "--"}</small>
        </div>`).join("");
}
...
function showRainAlertReason(cityName) {
    ...
    if (!item) return window.alert("لا توجد بيانات لهذا التنبيه");
    const maxScore = Math.max(Number(item.score || 0), Number(item.forecast24Score || 0), Number(item.forecast72Score || 0));
    window.RainGuardDecisionEngine?.renderAIDecisionReport?.(item, "aiDecisionReportBoxAlerts");
    window.RainGuardEventBus?.aiDecision?.({ city: item.name, stage: item.stage, label: item.label, decisionScore: maxScore, floodRiskScore: item.floodRiskScore, reason: "تم عرض سبب التنبيه" });
    window.alert("سبب التنبيه:\n\n" +
        "المدينة: " + item.name + "\n" +
...
function startRainAlertLifecycleMonitor() {
    if (rainAlertLifecycleInterval) clearInterval(rainAlertLifecycleInterval);
    rainAlertLifecycleInterval = setInterval(async () => {
        try { await runSmartMultiCityBackgroundCheck?.(true); }
        catch (e) { console.warn("Rain Alert Lifecycle monitor skipped:", e); }
...
function publishMultiCityResultsToEventBus(results) {
    ...
    results.forEach(city => {
        ...
        const payload = { name: city.name || city.city || "غير محدد", score, forecast24Score: forecast24, forecast72Score: forecast72, floodRiskScore: flood, risk: maxScore, updatedAt: Date.now() };
        window.RainGuardEventBus.updateCity(payload);
        if (score >= 30 || forecast24 >= 30 || forecast72 >= 30) window.RainGuardEventBus.rainAlert(payload);
        if (flood >= 30) window.RainGuardEventBus.floodAlert(payload);
        const report = window.RainGuardDecisionEngine?.buildAIDecisionReport?.(city);
        if (report) window.RainGuardEventBus.aiDecision({ city: report.city, decisionScore: report.score, confidence: report.confidence, decision: report.decision?.level, label: report.decision?.label });
    });
}

function syncResultsWithV11Engines(results) {
    if (!Array.isArray(results)) return;
    saveRainAlertLifecycle?.(results);
    publishMultiCityResultsToEventBus?.(results);
    window.RainGuardAIReasoning?.explainNational?.(results);
    window.RainGuardStormEngine?.publishStormAnalysis?.(results);
    window.RainGuardAnalyticsEngine?.renderAnalytics?.(results);
}
...
async function runSmartMultiCityBackgroundCheck(force = false) {
    ...
    try {
        ...
        syncResultsWithV11Engines?.(results);
        saveSmartMultiCityHistory?.(results.slice(0, SMART_MULTI_CITY_TOP_LIMIT || 20));
...
function updateNationalStatus(results) {
    const box = document.getElementById("nationalStatus") || document.getElementById("nationalStatusBox");
    if (!box) return;
    ...
    const maxFlood = Math.max(...results.map(city => Number(city.floodRiskScore || 0)), 0);
    const maxRain = Math.max(...results.map(city => Math.max(Number(city.score || 0), Number(city.forecast24Score || 0), Number(city.forecast72Score || 0))), 0);

    if (maxFlood >= 80) {
        box.innerHTML = `🚨 الحالة الوطنية: خطر مرتفع<div style="font-size:13px;color:#fecaca;margin-top:6px;">توجد مؤشرات سيول شديدة في بعض المدن.</div>`;
        box.style.borderColor = "#ef4444";
        return;
    }
    if (maxFlood >= 70) {
        box.innerHTML = `🔴 الحالة الوطنية: تحذير سيول<div style="font-size:13px;color:#fecaca;margin-top:6px;">توجد مدينة أو أكثر بمؤشر سيول مرتفع يحتاج متابعة عاجلة.</div>`;
        box.style.borderColor = "#ef4444";
        return;
    }
    if (maxFlood >= 55) {
        box.innerHTML = `🟠 الحالة الوطنية: مراقبة سيول<div style="font-size:13px;color:#fde68a;margin-top:6px;">توجد مدن تستحق المتابعة بسبب أمطار متوقعة ومؤشرات سيول.</div>`;
        box.style.borderColor = "#f59e0b";
        return;
    }
    if (maxRain >= 30) {
        box.innerHTML = `🔵 الحالة الوطنية: مراقبة أمطار<div style="font-size:13px;color:#bfdbfe;margin-top:6px;">توجد مدن عليها مؤشرات مطر تحتاج متابعة.</div>`;
...
async function getRainViewerRadarFusionV1(lat, lon) {
    try {
        ...
        if (!data.radar || !data.radar.past || data.radar.past.length === 0) {
            return { radarAvailable: false, radarIntensity: 0, radarSource: "RainViewer", note: "لا توجد بيانات رادار متاحة" };
        }
        ...
        const tileUrl = `https://tilecache.rainviewer.com${latestRadar.path}/256/${z}/${x}/${y}/1/1_1.png`;
        ...
        return new Promise(resolve => {
            ...
            img.onload = () => {
                try {
                    ...
                    const intensity = Math.max(px[0], px[1], px[2]);
                    resolve({ radarAvailable: true, radarIntensity: intensity, radarSource: "RainViewer", radarTime: latestRadar.time, radarPath: latestRadar.path, tileUrl });
                } catch (error) {
                    resolve({ radarAvailable: true, radarIntensity: 0, radarSource: "RainViewer", radarTime: latestRadar.time, radarPath: latestRadar.path, tileUrl, note: "تعذر قراءة بكسل الرادار" });
                }
            };
            img.onerror = () => resolve({ radarAvailable: true, radarIntensity: 0, radarSource: "RainViewer", radarTime: latestRadar.time, radarPath: latestRadar.path, tileUrl, note: "تعذر تحميل بلاطة الرادار" });
            img.src = tileUrl;
        });
    } catch (error) {
        console.error("Radar Fusion V1 Error:", error);
        return { radarAvailable: false, radarIntensity: 0, radarSource: "RainViewer", note: "فشل الاتصال بالرادار" };
    }
}
...
window.onload = function () {
    ...
    if (cachedResults.length > 0) {
        ...
        syncResultsWithV11Engines?.(cachedResults);
        console.log("Loaded cached MultiCity:", cachedResults.length);
    }

    try { initMap?.(); } catch (e) { console.warn("initMap skipped:", e); }
    try { renderSmartMultiCityHistory?.(); } catch (e) { console.warn("renderSmartMultiCityHistory skipped:", e); }
    try { renderRainAlertLifecycle?.(); startRainAlertLifecycleMonitor?.(); } catch (e) { console.warn("Rain Alert Lifecycle skipped:", e); }

    try {
        window.RainGuardEventBus?.autoRender?.("liveEventBusBox", 20);
        window.RainGuardEventBus?.systemStatus?.({ name: "RainGuard AI", status: "Application loaded", score: 100 });
    } catch (e) { console.warn("Event Bus init skipped:", e); }
    ...
    setTimeout(() => {
        try {
            ...
            if (rainCard) { rainCard.onclick = () => window.openFirstRainCity?.(); rainCard.style.cursor = "pointer"; }
            if (floodCard) { floodCard.onclick = () => window.openFirstFloodCity?.(); floodCard.style.cursor = "pointer"; }
            if (cloudCard) { cloudCard.onclick = () => showCloudCities?.(); cloudCard.style.cursor = "pointer"; }
        } catch (e) { console.warn("Summary card click handlers skipped:", e); }
    }, 1200);

    setTimeout(async () => {
        try {
            await checkRain?.(lastLat || 21.5433, lastLon || 39.1728, lastName || "جدة", false);
        } catch (e) { console.warn("Initial checkRain skipped:", e); }
    }, 1600);

    setTimeout(async () => {
        try {
            const results = await runSmartMultiCityBackgroundCheck?.(true);
            if (Array.isArray(results)) syncResultsWithV11Engines?.(results);
        } catch (e) { console.warn("Initial MultiCity check skipped:", e); }
    }, 2800);

    setTimeout(() => {
        try {
            if (isBackgroundMonitorEnabled?.()) startBackgroundRainMonitoring?.();
            else updateBackgroundMonitorStatus?.("جاهزة للتشغيل");
        } catch (e) { console.warn("Background rain monitoring skipped:", e); }
    }, 5000);

    setTimeout(() => {
        try { loadPredictionAnalytics?.(); } catch (e) { console.warn("Prediction analytics skipped:", e); }
        try { loadDashboardData?.(); } catch (e) { console.warn("Dashboard data skipped:", e); }
    }, 6500);
};
console.log("APP LOADED");
...
const oldRunSmartMultiCityBackgroundCheck = window.runSmartMultiCityBackgroundCheck || runSmartMultiCityBackgroundCheck;
...
window.runSmartMultiCityBackgroundCheck = async function (force = true) {
    ...
    syncResultsWithV11Engines?.(window.lastMultiCityResults);
    return window.lastMultiCityResults;
};
